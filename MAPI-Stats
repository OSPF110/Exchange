<#
# MAPI-Stats.ps1 -ErrorReport -Outpath $env:USERPROFILE\Documents -E16CU4orLater
.SYNOPSIS
...
Begin{
    ...
    if (!($Localpath)) {
        # get CAS servers
        if ($E16CU4orLater) {
            [array]$Servers += GetExchServer -Role 54,16439 -ADSites $ADSite | Where-Object {$_.Properties.versionnumber -ge '1942061725'}
        }
        else{
            [array]$Servers += GetExchServer -Role 54,16439 -ADSites $ADSite | Where-Object {$_.Properties.versionnumber -lt '1942061725'}
        }
        if ($SpecifiedServers) {
...
USING
TO_STRING(TO_TIMESTAMP(EXTRACT_PREFIX(REPLACE_STR([#Fields: datetime],'T',' '),0,'.'), 'yyyy-MM-dd hh:mm:ss'),'yyMMdd') AS Day,
TO_LOWERCASE(ClientSoftware) AS Client,
TO_LOWERCASE(ClientSoftwareVersion) AS Version
...
WHERE ClientSoftware IS NOT NULL AND Day IS NOT NULL
